...
function TrayBeat({ onNext }: { onNext: () => void }) {
  const [shown, setShown] = useState(false);
  // the beat reacts to the REAL tray appearing — whether via the keys or "Show me"
  useEffect(() => {
    const un = listen("tray:open", () => setShown(true));
    return () => {
      void un.then((f) => f());
    };
  }, []);
  ...
  return (
    <>
      ...
      <motion.div variants={rise} className="relative flex items-center gap-2">
        ...
        <motion.div className="flex items-center gap-2" animate={shown ? { scaleY: [1, 0.76, 1], y: [0, 5, 0] } : { y: [0, -6, 0] }} transition={shown ? { duration: 0.42, ease: OUT } : { duration: 1.8, repeat: Infinity, ease: "easeInOut" }}>
          <Keycap>⌥</Keycap>
          <span className="text-[20px] font-bold text-[#b3adbf]">+</span>
          <Keycap>⇧</Keycap>
          <span className="text-[20px] font-bold text-[#b3adbf]">+</span>
      ...
      <Rise className="mt-7 text-[24px] font-extrabold tracking-[-0.03em] text-[#1f2024]">
        <AnimatePresence mode="wait">
          <motion.span key={shown ? "open" : "intro"} initial={{ opacity: 0, y: 8, filter: "blur(5px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={{ opacity: 0, y: -6, filter: "blur(5px)" }} transition={{ duration: 0.26, ease: OUT }} className="inline-block">
            {shown ? "There's your tray" : "One more: the tray"}
          </motion.span>
        </AnimatePresence>
      </Rise>
      <Rise className="mt-1.5 max-w-[400px] text-[13.5px] leading-relaxed text-[#6b6577]">
        {shown
          ? "It floats beside your work. Drop in files and images, sort them into lanes, then send a whole lane to your board in one step."
          : "A staging area for things on the move. Press ⌥⇧Space and it appears beside your work."}
      </Rise>
      ...
      {!shown && (
        <Rise className="mt-7 flex flex-col items-center gap-2.5">
          <button onClick={() => void invoke("show_tray")} className="text-[12.5px] font-semibold text-[#6b4ea8] transition-colors hover:text-[#1f2024]">
            Show me instead
          </button>
          <button onClick={onNext} className="text-[12.5px] font-medium text-[#9a93a8] transition-colors hover:text-[#1f2024]">
            Skip for now
          </button>
        </Rise>
      )}
